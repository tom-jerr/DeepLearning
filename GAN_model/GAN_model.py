...
for epoch in range(num_epoch):
    for i, (img, _) in enumerate(dataloader):
        num_img = img.size(0)
        # =================train discriminator
        real_img = torch.tensor(img).cuda()
        real_label = torch.tensor(torch.ones(num_img, 1)).cuda()
        fake_label = torch.tensor(torch.zeros(num_img, 1)).cuda()
        ...
        z = torch.tensor(torch.randn(num_img, z_dimension)).cuda()
        fake_img = G(z)
        fake_out = D(fake_img)
...
# torch.save(G.state_dict(), 'models/generatorConv.pth')
# torch.save(D.state_dict(), 'models/discriminatorConv.pth')
torch.save(G, 'models/generatorConv.pth')
torch.save(D, 'models/discriminatorConv.pth')
